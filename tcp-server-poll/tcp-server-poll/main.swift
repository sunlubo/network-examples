...
var fds = [pollfd]()
fds.append(pollfd(fd: server_fd, events: Int16(POLLIN), revents: 0))
ret = poll(&fds, nfds_t(fds.count), 10000) // int poll(struct pollfd fds[], nfds_t nfds, int timeout)
assert(ret != -1, "poll: \(errno)")
assert(ret !=  0, "poll: timeout")
for pfd in fds {
    print(pfd)
}
